...
CP=$CP:$JETS3T_HOME/jars/jets3t-0.7.1.jar
CP=$CP:$JETS3T_HOME/jars/jets3t-gui-0.7.1.jar
CP=$CP:$JETS3T_HOME/jars/uploader-0.7.1.jar
...
if [ `uname -s` = "Darwin" ]
then
  "$EXEC" $MAXMEM -classpath "$CP" -Xdock:name="JetS3t Uploader" org.jets3t.apps.uploader.Uploader
 else
  "$EXEC" $MAXMEM -classpath "$CP" org.jets3t.apps.uploader.Uploader
fi
